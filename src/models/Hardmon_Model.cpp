...
void Hardmon_Model::Hardmon_Invalid_State_Checking(void) {
    ...
    if (static_cast<uint16_t>(Hardmon_DW.is_Invalid_State_Checking) == Hardm_IN_Invalid_State_Inactive) {
...
void Hardmon_Model::Hardmon_Normal_Operation(void) {
    ...
    if ((Hardmon_DW.WD_Counter >= 4.0) && (Hardmon_U.uC_State != UC_State::Preset)) {
        // Transition: '<S1>:8'
        ...
        if (static_cast<uint16_t>(Hardmon_DW.is_Watchdog_Checking) == Hardmon_IN_WD_High) {
        ...
        if (static_cast<uint16_t>(Hardmon_DW.is_Ignition_Level_Checking) == Hard_IN_Ignition_State_Inactive) {
        ...
        if (static_cast<uint16_t>(Hardmon_DW.is_ESTOP_Level_Checking) == Hardmon_IN_ESTOP_State_Inactive) {
        ...
        if (static_cast<uint16_t>(Hardmon_DW.is_LVSS_Checking) == Hardm_IN_Invalid_State_Inactive) {
            ...
            // Entry 'count': '<S1>:84'
            Hardmon_DW.LVSS_Counter++;
        } else if ((Hardmon_U.LVSS_ON == Hardmon_U.LVSS_EN_uC) || (Hardmon_U.uC_State == UC_State::Fault)) {
            // Transition: '<S1>:80'
...
void Hardmon_Model::step() {
    ...
    if (static_cast<uint16_t>(Hardmon_DW.is_active_c3_Hardmon) == 0U) {
        ...
        case Hardmon_IN_MC_FAULT_INIT:
            ...
            if ((Hardmon_U.uC_State != UC_State::MC_Off) && (Hardmon_U.uC_State != UC_State::Preset)) {
                // Transition: '<S1>:121'
                ...
                // Outport: '<Root>/Inverter_DIS'
                Hardmon_Y.Inverter_DIS = true;
            } else if ((Hardmon_U.uC_State == UC_State::MC_Off) || (Hardmon_U.uC_State == UC_State::Preset)) {
                // Transition: '<S1>:123'
                Hardmon_DW.is_c3_Hardmon = Hardmon_IN_MC_POWERDOWN1;
