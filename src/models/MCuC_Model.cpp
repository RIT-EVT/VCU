...
void MCuC_Model::MCuC_Logic(const bool* LogicalOperator, const bool* OR, const bool* AND) {
    ...
    if ((*OR) && MCuC_DW.MC_Uncharged) {
        ...
        if (((*OR) && tmp) || (MCuC_U.ESTOP_LS_A && tmp) || ((!*LogicalOperator) && tmp)) {
            // Transition: '<S10>:439'
            // Transition: '<S10>:199'
            // Transition: '<S10>:400'
            MCuC_exit_internal_Logic();
...
void MCuC_Model::step() {
    ...
    OR = ((MCuC_U.GFDB_Isolation_State_CAN != rtCP_pooled4) || (!MCuC_U.Interlock) || OR
    ...
    AND = (MCuC_U.Brake_CAN && MCuC_U.Forward_EN_CAN && (MCuC_U.Throttle_CAN == rtCP_Constant_Value_c));
    ...
    // Gateway: MCuC_Chart
    if (MCuC_DW.isNotInit && (static_cast<uint16_t>(MCuC_DW.temporalCounter_i1) < 7)) {
    ...
    case MCuC_IN_Preset:
        ...
        if (static_cast<uint16_t>(MCuC_DW.temporalCounter_i1) >= 4) {
            ...
            // Outport: '<Root>/BMS_Contactor_Command_uC_CAN'
            MCuC_Y.BMS_Contactor_Command_uC_CAN = BMS_Contactor_Command::Open_Contactor;
        } else if (MCuC_DW.MC_ST_Complete && MCuC_DW.CAN_ST_Complete && MCuC_DW.LS_ST_Complete) {
            // Transition: '<S10>:320'
            // Exit Internal 'Preset': '<S10>:302'
            // Exit Internal 'LS': '<S10>:459'
            MCuC_DW.is_LS = 0;
