...
namespace VCU
{
    ...
    void MCuC_Model::MCuC_Model_Setup(void)
    {
        ...
        switch (MCuC_Model_DW.is_Setup) {
          case MCuC_Model_IN_CAN_A_ST:
            ...
            if (static_cast<uint16_t>(MCuC_Model_DW.is_CAN_A_ST) !=
                    MCuC_Model_IN_A_CAN_fails) {
                ...
                if (MCuC_Model_U.A_CAN_ST && (!MCuC_Model_U.MC_Status_3V3) &&
                        MCuC_Model_U.Ignition_3V3 &&
                        (!MCuC_Model_U.LVSS_Status_3V3)) {
                    // Transition: '<S1>:320'
                ...
                } else if ((!MCuC_Model_U.A_CAN_ST) && (static_cast<uint16_t>
                            (MCuC_Model_DW.temporalCounter_i1) >= 150U)) {
                    // Transition: '<S1>:434'
          ...
          case MCuC_Model_IN_MC_CAN_ST:
            ...
            if (static_cast<uint16_t>(MCuC_Model_DW.is_MC_CAN_ST) !=
                    MCuC_Model_IN_MC_CAN_fails) {
                MCuC_Model_B.MC_CAN_out = true;

                // Inport: '<Root>/MC_CAN_ST'
                // During 'MC_Transmit': '<S1>:421'
                if ((!MCuC_Model_U.MC_CAN_ST) && (static_cast<uint16_t>
                        (MCuC_Model_DW.temporalCounter_i1) >= 150U)) {
                    // Transition: '<S1>:428'
          ...
          default:
            ...
            switch (MCuC_Model_DW.is_MC_ST) {
              ...
              case MCuC_Model_IN_MC_OFF:
                ...
                if (MCuC_Model_U.MC_Status_3V3 && (static_cast<uint16_t>
    ...
    void MCuC_Model::MCuC_Model_MCuC(bool *Watchdog)
    {
        ...
        guard1 = false;
        if (MCuC_Model_U.HM_Fault && tmp_1 && tmp &&
                (!MCuC_Model_DW.Fault_Activated)) {
            // Transition: '<S1>:356'
            ...
            if (tmp_0 && tmp && (!MCuC_Model_DW.ESTOP_activated)) {
                ...
                // Inport: '<Root>/nReset'
                       ...
                       (static_cast<int16_t>(MCuC_Model_B.uC_State) != 10)) {
                // Transition: '<S1>:200'
                ...
                switch (MCuC_Model_DW.is_MCuC_Logic) {
                  case MCuC_Model_IN_Contactor_Closed:
                    ...
                    if (MCuC_Model_U.Start_CAN && MCuC_Model_U.Forward_EN_CAN &&
                        MCuC_Model_U.Brake_CAN && (MCuC_Model_U.Throttle_CAN ==
                            0)) {
                        // Transition: '<S1>:17'
        ...
        if (static_cast<uint16_t>(MCuC_Model_DW.is_Watchdog) ==
                MCuC_Model_IN_WD0) {
            *Watchdog = false;
